...
class TargetParser final {
  public:
    ...
    std::unique_ptr<DmTarget> Next() {
	    ...
	    if (!HasArgs(5)) {
		std::cerr << "Expected: <keyslot> <cipher> <key_size> <device_path> <sector_offset>" << std::endl;
	        return nullptr;
	  }
	  ...
	  std::string keyslot = NextArg();
          ...
          table << keyslot << " " << cipher << " " << key_size << " " << dev_path << " " << sector_offset;
          // return std::make_unique<DmTarget>(target_type, start_sector, num_sectors, table.str());
          return std::make_unique<DefaultKeyTarget>(target_type, start_sector, num_sectors, table.str());
       } else if (target_type == "crypt") {
	    if (!HasArgs(5)) {
		std::cerr << "Expected: <cipher> <key> <iv_offset> <device_path> <sector_offset>" << std::endl;
	        return nullptr;
	    }
	...
	std::ostringstream table;
	table << cipher << " " << key << " " << iv_offset << " " << dev_path << " " << sector_offset;
	return std::make_unique<CryptTarget>(start_sector, num_sectors, table.str());
