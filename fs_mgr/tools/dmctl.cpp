...
static int Usage(void) {
  ...
  std::cerr << std::endl;
  std::cerr << "-f file reads command and all parameters from named file"
            << std::endl;
  std::cerr << std::endl;
  std::cerr << "Target syntax:" << std::endl;
  std::cerr << "  <target_type> <start_sector> <num_sectors> [target_data]"
            << std::endl;
  return -EINVAL;
}
...
public:
  DefaultKeyTarget(const std::string &type, uint64_t start, uint64_t num,
                   const std::string &params)
      : DmTarget(start, num), type_(type), parameters_(params) {

    // Debug log to verify the table string
    std::cerr << "Creating default-key target with parameters: " << parameters_
              << std::endl;
  }
...
public:
  CryptTarget(uint64_t start, uint64_t num, const std::string &params)
    ...
    std::cerr << "Creating crypt target with parameters: " << parameters_
              << std::endl;
  }
...
public:
  TargetParser(int argc, char **argv)
      : arg_index_(0), argc_(argc), argv_(argv) {}

  bool More() const { return arg_index_ < argc_; }
  std::unique_ptr<DmTarget> Next() {
    if (!HasArgs(3)) {
      std::cerr << "Expected <target_type> <start_sector> <num_sectors>"
                << std::endl;
      return nullptr;
    }
    ...
    if (!android::base::ParseUint(NextArg(), &num_sectors) || !num_sectors) {
      std::cerr << "Expected non-zero sector count, got: " << PreviousArg()
                << std::endl;
      return nullptr;
    }
    ...
    if (target_type == "zero") {
      ...
      if (!android::base::ParseUint(NextArg(), &physical_sector)) {
        std::cerr << "Expected sector, got: \"" << PreviousArg() << "\""
                  << std::endl;
        return nullptr;
      }
      return std::make_unique<DmTargetLinear>(start_sector, num_sectors,
                                              block_device, physical_sector);
    } else if (target_type == "android-verity") {
      if (!HasArgs(2)) {
        std::cerr
            << "Expected \"android-verity\" <public-key-id> <block_device>"
            << std::endl;
        return nullptr;
      }
      std::string keyid = NextArg();
      std::string block_device = NextArg();
      return std::make_unique<DmTargetAndroidVerity>(start_sector, num_sectors,
                                                     keyid, block_device);
    } else if (target_type == "striped") {
      if (!HasArgs(3)) {
        std::cerr << "Expected \"striped\" <block_device0> <block_device1> "
                     "<chunksize>"
                  << std::endl;
        return nullptr;
      }
      ...
      if (!android::base::ParseUint(NextArg(), &chunk_size)) {
        std::cerr << "Expected start sector, got: " << PreviousArg()
                  << std::endl;
        return nullptr;
      }
      return std::make_unique<DmTargetStripe>(
          start_sector, num_sectors, chunk_size, block_device0, block_device1);
      ...
      std::string block_device = NextArg();
      return std::make_unique<DmTargetBow>(start_sector, num_sectors,
                                           block_device);
      ...
      if (!HasArgs(4)) {
        std::cerr << "Expected \"snapshot\" <block_device> <block_device> "
                     "<mode> <chunk_size>"
                  << std::endl;
        return nullptr;
      }
      ...
      return std::make_unique<DmTargetSnapshot>(
          start_sector, num_sectors, base_device, cow_device, mode, chunk_size);
    } else if (target_type == "snapshot-merge") {
      if (!HasArgs(3)) {
        std::cerr << "Expected \"snapshot-merge\" <block_device> "
                     "<block_device> <chunk_size>"
                  << std::endl;
        return nullptr;
      }
      std::string base_device = NextArg();
      ...
      if (!HasArgs(4)) {
        std::cerr << "Expected \"thin-pool\" <metadata dev> <data dev> <data "
                     "block size> "
      ...
      return std::make_unique<DmTargetThinPool>(
          start_sector, num_sectors, metadata_dev, data_dev, data_block_size,
          low_water_mark);
      ...
      table << keyslot << " " << cipher << " " << key_size << " " << dev_path
            << " " << sector_offset;
      ...
      return std::make_unique<DmTargetThin>(start_sector, num_sectors, pool_dev,
                                            dev_id);
    } else {
...
static int DmCreateCmdHandler(int argc, char **argv) {
  if (argc < 1) {
    std::cerr
        << "Usage: dmctl create <dm-name> [--suspended] [-ro] <targets...>"
        << std::endl;
    return -EINVAL;
  }
  ...
  if (!dm.CreateEmptyDevice(name)) {
    std::cerr << "Failed to create device-mapper device with name: " << name
              << std::endl;
    return -EIO;
  }
...
static int DmReplaceCmdHandler(int argc, char **argv) {
  ...
  DeviceMapper &dm = DeviceMapper::Instance();
  if (!dm.LoadTable(name, table_args->table)) {
    std::cerr << "Failed to replace device-mapper table to: " << name
              << std::endl;
    return -EIO;
  }
  if (!table_args->suspended && !dm.ChangeState(name, DmDeviceState::ACTIVE)) {
...
static int DmListTargets(DeviceMapper &dm, [[maybe_unused]] int argc,
  ...
  for (const auto &target : targets) {
    std::cout << std::left << std::setw(20) << target.name() << " : "
              << target.version() << std::endl;
  }
...
static int DmListDevices(DeviceMapper &dm, int argc, char **argv) {
  ...
  for (const auto &dev : devices) {
    std::cout << std::left << std::setw(20) << dev.name() << " : "
              << dev.Major() << ":" << dev.Minor() << std::endl;
    if (verbose) {
...
static const std::map<std::string,
                      std::function<int(DeviceMapper &, int, char **)>>
    listmap = {
...
static int DmListCmdHandler(int argc, char **argv) {
  ...
  for (const auto &l : listmap) {
    if (l.first == argv[0])
      return l.second(dm, argc - 1, argv + 1);
  }
...
static int GetPathCmdHandler(int argc, char **argv) {
  ...
  if (!dm.GetDmDevicePathByName(argv[0], &path)) {
    std::cerr << "Could not query path of device \"" << argv[0] << "\"."
              << std::endl;
    return -EINVAL;
  }
...
static int GetUuidCmdHandler(int argc, char **argv) {
  ...
  if (!dm.GetDmDeviceUuidByName(argv[0], &uuid)) {
    std::cerr << "Could not query uuid of device \"" << argv[0] << "\"."
              << std::endl;
    return -EINVAL;
  }
...
static int InfoCmdHandler(int argc, char **argv) {
  ...
  std::cout << std::left << std::setw(spacing) << "activeTable"
            << ": " << std::boolalpha << info->IsActiveTablePresent()
            << std::endl;
  std::cout << std::left << std::setw(spacing) << "inactiveTable"
            << ": " << std::boolalpha << info->IsInactiveTablePresent()
            << std::endl;
...
static int DumpTable(const std::string &mode, int argc, char **argv) {
  ...
  std::cout << "Targets in the device-mapper table for " << argv[0] << ":"
            << std::endl;
  for (const auto &target : table) {
    std::cout << target.spec.sector_start << "-"
              << (target.spec.sector_start + target.spec.length) << ": "
...
static bool ReadFile(const char *filename, std::vector<std::string> *args,
  ...
  for (auto const &i : *args)
    arg_ptrs->push_back(const_cast<char *>(i.c_str()));
  return true;
}
