...
static void TestMmap(const unique_fd& fd, size_t size, int prot, void** region,
                     off_t off = 0) {
    ASSERT_TRUE(fd >= 0);
    ASSERT_TRUE(ashmem_valid(fd));
...
static void ForkTest(const unique_fd &fd, size_t size) {
    ...
    ASSERT_NO_FATAL_FAILURE(TestMmap(fd, size, PROT_READ | PROT_WRITE,
                                     &region1));
    ...
    if (!pid) {
        ...
        void *region2 = mmap(nullptr, size, PROT_READ | PROT_WRITE, MAP_SHARED,
                             fd, 0);
...
static void FileOperationsTest(const unique_fd &fd, size_t size) {
    ...
    ASSERT_NO_FATAL_FAILURE(TestMmap(fd, dataSize, PROT_READ | PROT_WRITE,
                                     &region, holeSize));
    ...
    for (const auto& cfg : seeks) {
        ...
        ASSERT_EQ(cfg.ret, off) << "lseek(" << cfg.offset << ", " << cfg.whence
                                << ") failed" << (errno ? ": " : "")
...
static void ForkMultiRegionTest(unique_fd fds[], int nRegions, size_t size) {
    ...
    for (int i = 0; i < nRegions; i++) {
        void* region = nullptr;
        ASSERT_NO_FATAL_FAILURE(TestMmap(fds[i], size, PROT_READ | PROT_WRITE,
                                         &region));
...
TEST(AshmemTest, ForkMultiRegionTest) {
    ...
    for (int i = 0; i < nRegions; i++) {
        ASSERT_NO_FATAL_FAILURE(TestCreateRegion(size, fds[i], PROT_READ |
                                                 PROT_WRITE));
    }
