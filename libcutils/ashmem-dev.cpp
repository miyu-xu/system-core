...
static bool __has_memfd_support() {
    ...
    if (ftruncate(fd, buf_size) == -1) {
        ALOGE("ftruncate(%s, %zd) failed to set memfd buffer size: %m, no memfd support", name,
              size);
        return false;
    }
    ...
    if (ashmem_size < 0) {
        ALOGE("ioctl(ASHMEM_GET_SIZE) failed: %m, no ashmem-memfd compat support");
        return false;
    } else if (ashmem_size != buf_size) {
        ALOGE("ioctl(ASHMEM_GET_SIZE) incorrectly returned: %d instead of %zd, no ashmem-memfd compat support");
        return false;
    }
