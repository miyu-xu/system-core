...
static std::string stringify_seals(int seals) {
    std::string str;

    if (seals & F_SEAL_FUTURE_WRITE) {
        str += "F_SEAL_FUTURE_WRITE";
    }

    if (seals & F_SEAL_FUTURE_EXEC) {
        if (!str.empty()) {
	    str += " | F_SEAL_FUTURE_EXEC";
	} else {
	    str += "F_SEAL_FUTURE_EXEC";
	}
    }
...
static int memfd_set_prot_region(int fd, int prot) {
    ...
    if (additional_seals && fcntl(fd, F_ADD_SEALS, additional_seals) == -1) {
        ALOGE("memfd_set_prot_region(%d, %d): adding %s seals failed: %s\n",
              fd, prot, stringify_seals(seals).c_str(), strerror(errno));
        return -1;
    }
