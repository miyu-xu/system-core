...
struct Stat {
  ...
  uint64_t cminflt; // (11) The number of minor faults that the process's
  ...
  uint64_t stime;  // (15) Amount of time that this process has been scheduled in kernel mode,
                   // measured in clock ticks.
  int64_t cutime;  // (16) Amount of time that this process's waited-for children have been
                   // scheduled in user mode, measured in clock ticks.
  int64_t cstime;  // (17) Amount of time that this process's waited-for children have been
                   // scheduled in kernel mode, measured in clock ticks.
  int64_t priority;  // (18) The standard priority number.
  int64_t
      nice;  // (19) The nice value, a value in the range 19 (low priority) to -20 (high priority).
  int64_t num_threads;  // (20) Number of threads in this process.
  int64_t itrealvalue;  // (21) The time in jiffies before the next SIGALRM is sent to the process
  ...
  uint64_t kstkesp;     // (29) The current value of ESP (stack pointer), as found in the kernel
                        // stack page for the process.
  uint64_t kstkeip;     // (30) The current EIP (instruction pointer).
  /*
  These fields are obsolete, use /proc/[pid]/status.

  unsigned long signal;        // (31) The bitmap of pending signals, displayed as a decimal number.
  unsigned long blocked;       // (32) The bitmap of blocked signals, displayed as a decimal number.
  unsigned long sigignore;     // (33) The bitmap of ignored signals, displayed as a decimal number.
  unsigned long sigcatch;      // (34) The bitmap of caught signals, displayed as a decimal number.
  */
  uint64_t wchan;  // (35) This is the "channel" in which the process is waiting.
  /*
  These fields are not maintained.

  unsigned long nswap;   // (36) Number of pages swapped.
  unsigned long cnswap;  // (37) Cumulative nswap for child processes.
  */
