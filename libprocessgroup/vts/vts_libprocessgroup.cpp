...
std::optional<bool> isMemcgV2Enabled() {
    ...
    std::cerr << "Error: Could not read " << CGROUP_V2_ROOT_PATH
              << "/cgroup.controllers: " << strerror(errno) << std::endl;
    return std::nullopt;
}
...
std::optional<bool> checkRootSubtreeState() {
    ...
    std::cerr << "Error: Could not read " << CGROUP_V2_ROOT_PATH
              << "/cgroup.subtree_control: " << strerror(errno) << std::endl;
    return std::nullopt;
}
...
class MemcgV2Test : public testing::Test {
  protected:
    void SetUp() override {
        if (std::optional<bool> memcgV2Enabled = isMemcgV2Enabled();
            !memcgV2Enabled.has_value() || !memcgV2Enabled.value()) {
            GTEST_SKIP() << "Memcg v2 not enabled";
            ASSERT_THAT(isMemcgV2Enabled(), testing::Optional(true));
        }
    }
};

class MemcgV2SubdirTest : public MemcgV2Test {
  protected:
    ...
    void SetUp() override {
        ...
        if (!*mRootSubtreeState) {
            ASSERT_TRUE(
                    ...
                    << "Could not enable memcg under root: " << strerror(errno);
        }
    }

    void TearDown() override {
        if (mRandDir) {
            EXPECT_TRUE(std::filesystem::remove(*mRandDir))
                    << "Could not remove temporary memcg v2 test directory";
        }

        if (!*mRootSubtreeState) {
            EXPECT_TRUE(
                    WriteStringToFile("-memory", CGROUP_V2_ROOT_PATH + "/cgroup.subtree_control"))
                    << "Could not disable memcg under root: " << strerror(errno);
        }
    }
...
TEST_F(MemcgV2SubdirTest, CanActivateMemcgV2) {
    // Make a new, temporary, randomly-named v2 cgroup in which we will attempt to activate memcg
    ...
    ASSERT_TRUE(ReadFileToString(randName + "/cgroup.controllers", &subtree_controllers));
    ASSERT_NE(subtree_controllers.find("memory"), std::string::npos)
            << "Memcg was not activated in child cgroup";
    ASSERT_TRUE(WriteStringToFile("+memory", randName + "/cgroup.subtree_control"))
