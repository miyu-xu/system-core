...
bool Modprobe::LoadModulesParallel(int num_threads, int mode) {
    ...
    std::vector<std::string> mods_to_load_parallel, mods_to_load_alone;
    std::vector<std::thread> threads;
    std::atomic<bool> ret(true), finish(false);;
    ...
    // Consumers load independent modules in parallel
    auto thread_function = [&] {
        while (!mods_with_deps.empty() && ret) {
            std::unique_lock lock(vector_lock);

            if (mods_to_load_alone.empty() && mods_to_load_parallel.empty()) {
                sleeping_threads++;
                ...
                cv_load_module.wait(lock, [&](){
                    return !mods_to_load_parallel.empty() ||
                           !mods_to_load_alone.empty() ||
                           finish.load(); });

                sleeping_threads--;
            }

            if (!mods_to_load_alone.empty()) {
                auto mod_to_load = std::move(mods_to_load_alone.back());
                mods_to_load_alone.pop_back();
                ret.store(ret.load() && LoadWithAliases(mod_to_load, true));
            } else if (!mods_to_load_parallel.empty()) {
                auto mod_to_load = std::move(mods_to_load_parallel.back());
                mods_to_load_parallel.pop_back();
    ...
    while (!mods_with_deps.empty()) {
        ...
        for (const auto& [it_mod, it_dep] : mods_with_deps) {
            ...
            if (mods_loading.find(cnd_last) == mods_loading.end()) {
                ...
                if (NeedLoadAlone(cnd_last))
                    mods_to_load_alone.emplace_back(cnd_last);
                else
                    mods_to_load_parallel.emplace_back(cnd_last);
            }
        }

        cv_load_module.notify_all();
        cv_update_module.wait(lock, [&](){ return mods_to_load_parallel.empty(); });

        if (!ret.load())
            break;

        std::lock_guard guard(module_loaded_lock_);
        // Remove loaded module form mods_with_deps
        for (const auto& module_loaded : module_loaded_)
            mods_with_deps.erase(module_loaded);

        // Remove loaded module form dependency list
...
bool Modprobe::NeedLoadAlone(const std::string& module)
{
