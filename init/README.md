...
* Property trigger is also checked when other triggers in the same
  action are checked. For example, `property:a=b && property:c=d` is
checked not only when property `a` gets a new value, but also property `c`
gets a new value (and of course when the one-time check is made).
