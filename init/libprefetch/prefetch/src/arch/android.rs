...
/// Ensure we are not running record on first boot after data wipe.
/// Check if a file specified exists to see if the device got wiped before the boot,
/// create a new file and exits early if the file does not exist.
pub fn skip_firstboot_after_datawipe(ready_path: &Path) -> Result<(), Error> {
    ...
    info!("Detected running on first boot after data wipe -- exiting.");
...
/// Equivalent to "not can_perform_replay", as being able to replay means
/// we do not need to record.
pub fn can_perform_record(pack_path: &Path, fingerprint_path: &Path) -> Result<bool, Error> {
    can_perform_replay(pack_path, fingerprint_path).map(|x| !x)
}
